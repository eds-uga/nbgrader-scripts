...
class ExchangeFetchAssignment(Exchange):
    ...
    def copy_files(self):
        ...
        ##################################################
        ###    EVERYTHING AFTER THIS POINT HAS BEEN    ###
        ###   ADDED FOR THE TIMED ASSIGNMENT MONITOR   ###
        ##################################################

        if self.do_timed():
            self.log.info("{} fetched, timer set".format(self.coursedir.assignment_id))

    def do_timed(self, timed = ["midterm", "final"]):
        """
        @author Shannon Quinn
        Custom function to handle timed assignments using nbgrader. This function 
        will write timestamps at fetch time to the exchange. Note that this won't
        necessarily prevent direct tampering.
        """
        if self.coursedir.assignment_id not in timed:
            return False

        # A bit of a hacky hack, admittedly.
        durations = {"midterm": 90, "final": 180}

        # Some imports.
        from datetime import datetime, timedelta
        import getpass

        # Here's the meat of it: get the current student and the timestamp,
        # record them to a file, and get out.
        student_id = getpass.getuser()
        now = datetime.now()
        dt = timedelta(minutes = durations[self.coursedir.assignment_id])
        fetchfile = os.path.join(self.root, "{}.txt".format(student_id))
        entry = "FETCH : {} :: DEADLINE : {}\n".format(
            now.strftime("%Y-%m-%d %H:%M:%S"),
            (now + dt).strftime("%Y-%m-%d %H:%M:%S"))
        with open(fetchfile, "a") as f:
            f.write(entry)
        return True
